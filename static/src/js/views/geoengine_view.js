        ...
        switch (cfg.geo_repr) {
            ...
            default: // basic
                ...
                return {/*
                    ...
                    style: self.createMultiSymbolStyle(defaultStyle, cfg.symbols, self.getBasicCircleRadius(cfg, data)),
                    legend: ''
                };
        }
    },

    createMultiSymbolStyle: function(defaultStyle, symbols, radius) {
          ...
          if(typeof symbol == "undefined" || symbols.length == 0){
            var defaultStyleFunction = function(feature, resolution) {
                 ...
                 defaultStyle.text_.text_ = label_text;
                 return [defaultStyle];
            };
          ...
          var styleFunction = function(feature, resolution) {
            ...
            symbols.forEach(function(symbol){
                //{value: "Horeca", fieldname: "category", id: 3, img: "comshop/static/img/random.png"}
                var fieldValue = feature.get('attributes')[symbol.fieldname];

                //In case of Many2one
                if(Array.isArray(fieldValue)) {
                  fieldValue = fieldValue[1]; //we get the value
                }

                if (fieldValue && fieldValue == symbol.value) {
                  if(!styleCache[fieldValue]){
                    styleCache[fieldValue] = new ol.style.Style({
                      image: new ol.style.Circle({
                        fill: new ol.style.Fill({
                          color: symbol.color
                        }),
                  ...
                  rules.push(styleCache[fieldValue]);
                }
        ...
        if (_.isUndefined(this.map)){
            ...
            map = new ol.Map({
                ...
                view: new ol.View({
                    center: [0, 0],
                    zoom: 2,
                    projection: this.fields_view.geoengine_layers.projection ? this.fields_view.geoengine_layers.projection : "EPSG:3857"
                }),
